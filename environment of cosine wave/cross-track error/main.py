...
kejieshoukuandai = 0.4*0.01  * 0.25 #0.0125
...
L_2_max = (gongkuang+0.5)*10*shijianbu
...
Z_huatu_shangxian = max(Z) + 0.4 * 0.025

Z_huatu_xiaxian = min(Z) -  0.4 * 0.025
...
class Fuckingfly:
    ...
    def reset(self,seed_gei=None):
        ...
        self.X=np.linspace(0,xmax,self.duan+1)
    ...
    def step(self, action):
        ...
        if abs(xiangduigaodu - flyingh)<= kejieshoukuandai:
            
            xiangduigaodu = flyingh# 虚假的相对高度
        ...
        # if done == True and self.dijibu < allstep:
            
        #     shinengnow = 0            
        # else:
            
        #     shinengnow,_ = shinengjisuan(xiangduigaodu,xhua_n)
    ...
    # def shinengjisuan_shape(self,xhua_n,zhua_n,x_last,z_last):   ### lingyifang的子函数
    ...
    def suan_lingyifang(self):
        ...
        for num in range(len(xhua)-2):
            ...
            AAA = self.shinengjisuan_shape(xhua[num+1],zhua[num+1],xhua[num],zhua[num])
...
#         print('v_tp',v_tp)
  ...
  #           print('xiangjiao.area',xiangjiao.area)
    ...
    def huoquzuihouyidian(self):
        
    
            xzuobiao = self.jilustates[-1,10]
            
            zzuobiao = -self.jilustates[-1,11]
                    
            jiao = self.jilustates[-1,0]
            
            gabove = 0.31*0.4

            
            xhua_n = xzuobiao + gabove*math.sin(jiao*math.pi/180)
            
            zhua_n = zzuobiao-gabove*math.cos(jiao*math.pi/180) 
            
            
            return xhua_n,zhua_n
        
        
    def suoyoubumianji(self):
        ...
        if len(xzuobiao)==(allstep+1):
            ...
            # dayutheone_x = fuckda_x[0]
            
            # dayutheone_z = fuckda_z[0]
        
            # cc = np.where(xhua <= theone_x)
            
            
            # fuckxiao_x = xhua[cc]
            
            # if fuckxiao_x.size == 0:
            ...
            # xiaoyutheone_x = fuckxiao_x[-1]
            
            # xiaoyutheone_z = fuckxiao_z[-1]   
...
def shinengjisuan(xiangduigaodu,x):
   
        fenmu = flyingh
            
        # if  fenmu>= xiangduigaodu:           
        
        #     cost_n =  1 - 0.5*abs((xiangduigaodu-fenmu)/fenmu)
        
        # if  fenmu < xiangduigaodu:           
            
        cost_n =  1 - np.clip(abs((abs(xiangduigaodu)-fenmu)/fenmu),0,1)
 
        # reward_bu = 0
        
        # jieshou = 0.4*0.01 #flyingh*0.1 #0.4*0.02
        
        # if fenmu-jieshou <=xiangduigaodu<=jieshou+fenmu:
            
        #     reward_bu = 1 - abs((xiangduigaodu-fenmu)/jieshou)
        ...
        # if  (0.4*0.5+fenmu)>= xiangduigaodu:
            
        #     cost_n =  (1 - abs((xiangduigaodu-fenmu)/fenmu))
            
        # if xiangduigaodu> (0.4*0.5+fenmu):
            
        #     cost_n = 0
   
        # reward_bu = 0
        
        # jieshou = 0.4*0.05 #flyingh*0.1 #0.4*0.02
        ...
        return cost_n**orderjieshu, 0 # reward_bu# reward_bu#0 #0.5*cost_n
...
def huatu(jilustates,  Tzhong,    flap_max,    initial,  shijianbu,xz, casenum_ori):
    ...
    axes1.set_ylim([Z_huatu_xiaxian, Z_huatu_shangxian])  
