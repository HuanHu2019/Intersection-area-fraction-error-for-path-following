...
def train_and_evaluate(args,initial,end):
    ...
    theshangxian_1_haolema = False
    
    theshangxian_2_haolema = False
    
    theshangxian_3_haolema = False
    
    theshangxian_4_haolema = False
    
    theshangxian_5_haolema = False    
    ...
    while not ((if_break_early and if_reach_goal)
        ...
        with torch.no_grad(): 
            ...
            if_reach_goal1,r1,neibudiedaidaolema,lingyifangjieguo,shangxiajiaocuo,zuihouyi_x,zuihouyi_y,guanjiandianzuoyou_area = evaluator1.evaluate_save(agent.act, agent.cri, steps, obj_a, obj_c,buffer)
        ...
        #if zongr <= rping or ((np.isclose(rping,allsteps) and dadaodianfeng_cishu < 2000)) :
        
        if zongr <= rping:
            
            
            
            x_list.append(zuihouyi_x)
            
            y_list.append(zuihouyi_y)
            ...
            if np.isclose(rping,allsteps,atol=1e-012):
                
                guanjiandianzuoyou_area_list.append(guanjiandianzuoyou_area)
                
                areamen_numpy = np.array(guanjiandianzuoyou_area_list) 
                
                np.save('areamen_numpy.npy',areamen_numpy)
                
                print('areamen_geshu',len(areamen_numpy))
                
                zuidamianjiwei_max = np.argmax(areamen_numpy)
                
                dianfengchaochusu_max = len(guanjiandianzuoyou_area_list) - (zuidamianjiwei_max + 1) 
                
                
                
                zuidamianjiwei_min = np.argmin(areamen_numpy)
                
                dianfengchaochusu_min = len(guanjiandianzuoyou_area_list) - (zuidamianjiwei_min + 1)       
                
                
                print('dianfengchaochusu_max',dianfengchaochusu_max)
                
                print('dianfengchaochusu_min',dianfengchaochusu_min)
                
                
                if dianfengchaochusu_max> theshangxian_1: # and dianfengchaochusu_min > theshangxian:
                    
                    if theshangxian_1_haolema == False:
                        
                        np.save('fir_stop',zongbu)
                        
                        np.save('fir_areamen_numpy.npy',areamen_numpy)
                        
                        np.save('fir' +initial_zifu+'to'+end_zifu+'pingjunzuida_zhi_he_bushu.npy',zuhe[1:,:])
                        
                        theshangxian_1_haolema = True
                    
                    if dianfengchaochusu_max> theshangxian_2:
                        
                        
                        if theshangxian_2_haolema == False:
                        
                            np.save('sec_stop',zongbu)
                            
                            np.save('sec_areamen_numpy.npy',areamen_numpy)
                            
                            np.save('sec' +initial_zifu+'to'+end_zifu+'pingjunzuida_zhi_he_bushu.npy',zuhe[1:,:])
                        
                            theshangxian_2_haolema = True
                        
                        if dianfengchaochusu_max> theshangxian_3:
                            
                            if theshangxian_3_haolema == False:
                            
                                np.save('third_stop',zongbu)
                            
                                theshangxian_3_haolema = True  
                                
                                np.save('third_areamen_numpy.npy',areamen_numpy)
                                
                                np.save('third' +initial_zifu+'to'+end_zifu+'pingjunzuida_zhi_he_bushu.npy',zuhe[1:,:])
                            
                            
                            if dianfengchaochusu_max> theshangxian_4:
                                
                                if theshangxian_4_haolema == False:
                                
                                    np.save('forth_stop',zongbu)
                                
                                    theshangxian_4_haolema = True  
                                    
                                    np.save('forth_areamen_numpy.npy',areamen_numpy)
                                    
                                    np.save('forth' +initial_zifu+'to'+end_zifu+'pingjunzuida_zhi_he_bushu.npy',zuhe[1:,:])                            
                            
                            
                                if dianfengchaochusu_max> theshangxian_5:
                                    
                                    if theshangxian_5_haolema == False:
                                    
                                        np.save('fifth_stop',zongbu)
                                    
                                        theshangxian_5_haolema = True  
                                        
                                        np.save('fifth_areamen_numpy.npy',areamen_numpy)
                                        
                                        np.save('fifth' +initial_zifu+'to'+end_zifu+'pingjunzuida_zhi_he_bushu.npy',zuhe[1:,:])                            
                                
                                                        
                            
                            
                            
                                    if dianfengchaochusu_max> theshangxian_6:
        
                                        
                                        np.save('sixth_stop',zongbu)
        ...
        #print('dianfengchaochusu',dianfengchaochusu)
        
        #print('guanjiandianzuoyou_area_list',guanjiandianzuoyou_area_list)
...
class Evaluator:
    ...
    def evaluate_save(self, act,cri, steps, obj_a, obj_c,buffer) -> bool:
        ...
        if 1:
            ...
            r_avg, r_std, s_avg, s_std,lingyifangjieguo,shangxiajiaocuo,zuihouyi_x,zuihouyi_y,guanjiandianzuoyou_area= self.get_r_avg_std_s_avg_std(rewards_steps_list)
        ...
        return if_reach_goal,r_avg,neibudiedaidaolema,lingyifangjieguo,shangxiajiaocuo,zuihouyi_x,zuihouyi_y,guanjiandianzuoyou_area
    ...
    def get_r_avg_std_s_avg_std(rewards_steps_list):
        ...
        r_avg, s_avg,lingyifangjieguo_avg,jiaocuo_avg,last_x,last_y,guanjiandianzuoyou_area = rewards_steps_ary.mean(axis=0)  # average of episode return and episode step
        r_std, s_std,_,_,_,_,_ = rewards_steps_ary.std(axis=0)  # standard dev. of episode return and episode step
        return r_avg, r_std, s_avg, s_std,lingyifangjieguo_avg,jiaocuo_avg,last_x,last_y,guanjiandianzuoyou_area
...
def get_episode_return(env, act, device,buffer,casenum) -> (float, int):
    ...
    lingyifang = 100#env.suan_lingyifang()
    
    shangxiajiaocuo = 100 #env.suan_shencha_zhuhanshu()
    
    last_x,last_y = 100,100  #env.huoquzuihouyidian()
    
    guanjiandianzuoyou_area = env.suoyoubufenzihe()
    
    
    episode_return = getattr(env, 'episode_return', episode_return)
    return episode_return, episode_step + 1,lingyifang, shangxiajiaocuo, last_x, last_y,guanjiandianzuoyou_area
